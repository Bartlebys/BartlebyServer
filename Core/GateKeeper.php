...
class GateKeeper {
            ...
            if (array_key_exists(LEVEL_KEY, $rule)) {
                ...
                if (is_numeric($level)) {
                    ...
                    switch ($level) {
                        ...
                        case PERMISSION_BY_IDENTIFICATION_AND_ACTIVATION:
                            return PERMISSION_BY_IDENTIFICATION_AND_ACTIVATION;
    ...
    private function _resultOfRule($level, $key, $rule) {
        switch ($level) {
            ...
            case PERMISSION_BY_IDENTIFICATION_AND_ACTIVATION:
                ...
                $result = ($this->_identityIsValid($this->_context->getSpaceUID())
                    && isset($this->_user)
                    && is_array($this->_user)
                    && array_key_exists('status',$this->_user)
                    &&$this->_user['status']=='actived');
    ...
    private function _identityIsValid($spaceUID) {
        ...
        $userID = $this->_context->getCurrentUserUID();
        if (isset($userID)) {
            $this->_context->consignIssue('User has been extracted userID='.$userID, __FILE__, __LINE__);
    ...
    private function _getDynamicPermissionForKey($key) {
        ...
        // The dynamic features may become deprecated for security reason
        return array();
    }
