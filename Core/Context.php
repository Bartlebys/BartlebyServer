...
class Context implements IAuthentified{
    ...
    /**
     * Grabs the observationUIDS
     * @return array of string the Observation UID.
     */
    public function getObservationUIDS(){
        $observationUIDS=NULL;
        if (array_key_exists(OBSERVATION_UIDS_KEY,$this->_allVariables)){
            $observationUIDS=$this->_allVariables[OBSERVATION_UIDS_KEY];
        }
    ...
    public function getUserID($spaceUID){
        ...
        $userUID=$this->_getUserIDFromCookie($spaceUID);
        if (isset($userUID)){
            return $userUID;
        }else{
            return NULL;
    ...
    private function _getUserIDFromKVI($spaceUID) {
        ...
        if (array_key_exists(Auth::kvidKey,$allHeader) && trim($allHeader[Auth::kvidKey])!= "") {
