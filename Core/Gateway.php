        ...
        if ($filePath == "") {
            ...
            try {
                ...
                if ($authorized === true) {
                    ...
                    if (class_exists($classForCallData)) {
                        // Instanciate Call Data
                        $callDataInstance = new $classForCallData($filteredParameters);
                    } else {
                        $infos [Configuration::INFORMATIONS_KEY] = 'Call data class not found: ' . $classForCallData;
                        $this->_response = new JsonResponse($infos, 405);
                        return; //  stop the execution flow.
                    }
                    if (class_exists($classForElement)) {
                        // We instantiate the class.
                        $instance = new $classForElement($callDataInstance, $this->_context);
                    } else {
                        $infos [Configuration::INFORMATIONS_KEY] = 'Class for endpoint or page not found: ' . $classForElement;
                        $this->_response = new JsonResponse($infos, 405);
                        return; //  stop the execution flow.
                    }
