...
require_once __DIR__ . '/RelayResponse.php';
...
if (!defined('PERMISSION_IS_STATIC')) {
    ...
    define('PERMISSION_BY_IDENTIFICATION_AND_ACTIVATION', 5); // PERMISSION_BY_IDENTIFICATION + user is Actived
    define('PERMISSION_RESTRICTED_TO_ENUMERATED_USERS', 6);
    define('PERMISSION_RESTRICTED_BY_QUERIES', 7);
    define('PERMISSION_RESTRICTED_TO_GROUP_MEMBERS', 8);
...
class Configuration {
    ...
    */
    ...
    ////////////////////////
    // MONGO WRITE CONCERNS
    ///////////////////////

    /**
     * Returns the Write Acknowledgement policy
     *
     * You can override this method in your configuration to variate the value per STAGES
     *
     * e.g: you can use Replication in production, not in development.
     *
     *  if ($this->_STAGE == Stages::PRODUCTION) {
     *      return array ( "w" => 2, "j" => true, "wtimeout" => 1500 );
     *  }else{
     *      return array ( "w" => 2, "j" => true);
     *  }
     *
     ...
     * @return array
     */
    public function getDefaultMongoWriteConcern(){
        return array (
            // the w option to request acknowledgment that the write operation has propagated
            // to a specified number of mongod instances or to mongod instances with specified tags.
            "w" => 1,
    ...
    /**
     * The relay function should be overriden according to your policy context
     * You should use the $phoneNumber and send a short messsage.
     * @param $toEmail
     * @param $toPhoneNumber
     * @param $title
     * @param $body
     ...
     */
    public  function relay($toEmail, $toPhoneNumber,$title,$body){
        // This is a placeholder
        // You should override the relay method and implement a valid connector
        $r=new RelayResponse();
        if  (mail ( $toEmail , $title , $body ) === true) {
            $r->success=true;
        }else{
            $r->informations='An opaque error has occured';
            $r->success=false;
        }
        return $r;
    }
