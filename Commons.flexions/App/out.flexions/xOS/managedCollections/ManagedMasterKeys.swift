...
public extension Notification.Name {
    public struct MasterKeys {
        /// Posted when the selected masterKeys changed
        public static let selectionChanged = Notification.Name(rawValue: "org.bartlebys.notification.MasterKeys.selectedMasterKeysChanged")
    }
}


// MARK: A  collection controller of "masterKeys"

// This controller implements data automation features.

@objc open class ManagedMasterKeys : ManagedModel,IterableCollectibleCollection{

    open var collectedType:Collectible.Type { return MasterKey.self }

    // Staged "masterKeys" identifiers (used to determine what should be committed on the next loop)
    fileprivate var _staged=[String]()

    // Store the  "masterKeys" identifiers to be deleted on the next loop
    fileprivate var _deleted=[String]()

    // Ordered UIDS
    fileprivate var _UIDS=[String]()

    // The "masterKeys" list (computed by _rebuildFromStorage and on operations)
    @objc fileprivate dynamic var _items=[MasterKey]()  {
        didSet {
    ...
    // The underlining "masterKeys" storage (serialized)
    ...
    // So we use a strongly typed `MasterKey for the storage
    // While the API deals with `Collectible` instances.
    fileprivate var _storage=[String:MasterKey]()

    fileprivate func _rebuildFromStorage(){
        self._UIDS=[String]()
        self._items=[MasterKey]()
    ...
    required public init(items:[MasterKey], within document:BartlebyDocument) {
        super.init()
    ...
    open func propagate(){
        ...
        if self.referentDocument == nil{
            glog("Document Reference is nil during Propagation on ManagedMasterKeys", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
        }
        #endif
        for item in self{
    ...
    open func generate() -> AnyIterator<MasterKey> {
        var nextIndex = -1
    ...
    open subscript(index: Int) -> MasterKey {
    ...
    open func indexOf(element:@escaping(MasterKey) throws -> Bool) rethrows -> Int?{
    ...
    */
    ...
    open func commitChanges(){
        if self._staged.count>0{
            var changedMasterKeys=[MasterKey]()
            for itemUID in self._staged{
                if let o:MasterKey = try? Bartleby.registredObjectByUID(itemUID){
                    changedMasterKeys.append(o)
                }
            }
            for changed in changedMasterKeys{
				if changed.commitCounter > 0 {
				    UpdateMasterKey.commit(changed, in:self.referentDocument!)
				}else{
				    CreateMasterKey.commit(changed, in:self.referentDocument!)
				}
        ...
        if self._deleted.count > 0 {
            var toBeDeletedItems=[MasterKey]()
            for itemUID in self._deleted{
                if let o:MasterKey = try? Bartleby.registredObjectByUID(itemUID){
                    toBeDeletedItems.append(o)
                }
            }
            for masterKey in toBeDeletedItems{
                DeleteMasterKey.commit(masterKey, from: self.referentDocument!)
                Bartleby.unRegister(masterKey)
            }
    ...
    override open class var collectionName:String{
        return MasterKey.collectionName
    }
    ...
    override open func setExposedValue(_ value:Any?, forKey key: String) throws {
        switch key {
            case "_storage":
                if let casted=value as? [String:MasterKey]{
    ...
    required public init(from decoder: Decoder) throws{
        ...
        try self.quietThrowingChanges {
			...
			self._storage = try values.decode([String:MasterKey].self,forKey:._storage)
    ...
    ///
    /// - Parameters:
    ///   - item: the MasterKey    ///   - commit: should we commit the `Upsertion`?
    /// - Returns: N/A
    ...
    /// Ads an MasterKey    ///
    /// - Parameters:
    ///   - item: the MasterKey    ///   - commit: should we commit the addition?
    ...
    open func append(_ items:[Collectible],commit:Bool, isUndoable:Bool){
        if let items  = items as? [MasterKey] {
            ...
            if isUndoable{
                ...
                if let undoManager: UndoManager = self.undoManager {
                    ...
                    if !undoManager.isUndoing {
                        undoManager.setActionName(NSLocalizedString("Add MasterKey", comment: "AddMasterKey undo action"))
                    }
                }
            }
            if commit==true {
               CreateMasterKeys.commit(items, in:self.referentDocument!)
            }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit:Bool=true,isUndoable:Bool) {
        if let item = item as? MasterKey{
            ...
            if commit==true {
               CreateMasterKey.commit(item, in:self.referentDocument!)
            }
            self.shouldBeSaved = true
        }
    }
    ...
    */
    open func removeObjectFromItemsAtIndex(_ index: Int, commit:Bool=true) {
      ...
      // Add the inverse of this invocation to the undo stack
        if let undoManager: UndoManager = self.undoManager {
            ...
            if !undoManager.isUndoing {
                undoManager.setActionName(NSLocalizedString("Remove MasterKey", comment: "Remove MasterKey undo action"))
            }
        }
    ...
    open func addObjectFrom(_ data:Data){
        do{
            if let masterKey:MasterKey = try self.referentDocument?.serializer.deserialize(data,register:true){
                if let owners = Bartleby.registredManagedModelByUIDs(masterKey.ownedBy){
                    for owner in owners{
                        // Re associate the relations.
                        if !owner.owns.contains(masterKey.UID){
                            owner.owns.append(masterKey.UID)
                        }
                    }
                }
                self.add(masterKey, commit: true, isUndoable:false)
            }
        }catch{
    ...
    open func removeObject(_ item: Collectible, commit:Bool=true){
        if let instance=item as? MasterKey{
    ...
    open func filteredCopy(_ isIncluded: (Collectible)-> Bool) -> CollectibleCollection{
        let filteredCollection=ManagedMasterKeys()
    ...
    open weak var arrayController:NSArrayController? {
        ...
        didSet{
            //self.referentDocument?.setValue(self, forKey: "masterKeys")
            arrayController?.objectClass=MasterKey.self
            arrayController?.entityName=MasterKey.className()
    ...
    // e.g: referentDocument.masterKeys.arrayController?.setSelectedObjects(masterKeys)
    // Do not use document.masterKeys.selectedMasterKeys=masterKeys
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if let keyPath = keyPath, let object = object {
            if keyPath=="selectionIndexes" &&  (object as? NSArrayController) == self.arrayController {
                if let items = self.arrayController?.selectedObjects as? [MasterKey] {
                    self.selectedMasterKeys=items
                }
            }
        }
    }
    ...
    fileprivate var _selectedUIDS:[String]{
        set{
            Bartleby.syncOnMain {
                if let masterKeys = self.selectedMasterKeys {
                    let _selectedUIDS:[String]=masterKeys.map({ (masterKey) -> String in
                        return masterKey.UID
                    })
                    let encodedUIDS = (try? JSON.encoder.encode(_selectedUIDS)) ?? "[]".data(using: Default.STRING_ENCODING)!
                    self.referentDocument?.metadata.statesDictionary[selectedMasterKeysUIDSKeys] = encodedUIDS
                    self.referentDocument?.hasChanged()
                }
            }
        }
        get{
            return Bartleby.syncOnMainAndReturn{ () -> [String] in
                if let data = self.referentDocument?.metadata.statesDictionary[self.selectedMasterKeysUIDSKeys]{
    ...
    open let selectedMasterKeysUIDSKeys="selectedMasterKeysUIDSKeys"

    // Note :
    // If you use an ArrayController & Bartleby automation
    // to modify the current selection you should use the array controller
    // e.g: referentDocument.masterKeys.arrayController?.setSelectedObjects(masterKeys)
    @objc dynamic open var selectedMasterKeys:[MasterKey]?{
        didSet{
            Bartleby.syncOnMain {
                if let masterKeys = selectedMasterKeys {
                    let UIDS:[String]=masterKeys.map({ (masterKey) -> String in
                        return masterKey.UID
                    })
                    self._selectedUIDS = UIDS
                }
                NotificationCenter.default.post(name:NSNotification.Name.MasterKeys.selectionChanged, object: nil)
            }
        }
    }

    // A facility
    open var firstSelectedMasterKey:MasterKey? { return self.selectedMasterKeys?.first }
