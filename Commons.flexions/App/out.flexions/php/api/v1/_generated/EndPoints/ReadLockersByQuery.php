...
class  ReadLockersByQueryCallData extends MongoCallDataRawWrapper {
 ...
 class  ReadLockersByQuery extends MongoEndPoint {

     function call() {
        /* @var ReadLockersByQueryCallData */
        $parameters=$this->getModel();
        $db=$this->getDB();
        /* @var \MongoCollection */
        $collection = $db->lockers;
      $q = $parameters->getValueForKey(ReadLockersByQueryCallData::query);
       if(!isset($q)){
           return new JsonResponse(VOID_RESPONSE,417);
       }
       $f=$parameters->getValueForKey(ReadLockersByQueryCallData::result_fields);
        try {
            $r=array();
            if(isset($f)){
            ...
            // Sort ?
            $s=$parameters->getCastedDictionaryForKey(ReadLockersByQueryCallData::sort);
