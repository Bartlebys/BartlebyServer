...
class  ReadBoxesByQueryCallData extends MongoCallDataRawWrapper {
 ...
 class  ReadBoxesByQuery extends MongoEndPoint {

     function call() {
        /* @var ReadBoxesByQueryCallData */
        $parameters=$this->getModel();
        $db=$this->getDB();
        /* @var \MongoCollection */
        $collection = $db->boxes;
      $q = $parameters->getValueForKey(ReadBoxesByQueryCallData::query);
       if(!isset($q)){
           return new JsonResponse(VOID_RESPONSE,417);
       }
       $f=$parameters->getValueForKey(ReadBoxesByQueryCallData::result_fields);
        try {
            $r=array();
            if(isset($f)){
            ...
            // Sort ?
            $s=$parameters->getCastedDictionaryForKey(ReadBoxesByQueryCallData::sort);
