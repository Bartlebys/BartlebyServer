    ...
    function call() {
        ...
        try {
            ...
            if (isset($r)) {
                ...
                $reinjectPassword=false;
                if (!array_key_exists('password',$user)){
                    $reinjectPassword=true;
                }

                if (array_key_exists('password', $user)) {
                    $foundPassword = $user['password'];
                    if ($foundPassword == '' || $foundPassword == $this->getConfiguration()->salt('')) {
                        $reinjectPassword = true;
                    }
                }
                ...
                if ($reinjectPassword){
                    if (array_key_exists('password',$r)){
                        $password=$r['password'];
                        KeyPath::setValueByReferenceForKeyPath($parameters,"user.password",$password);
                    }
                }
