...
class  ReadBlocksByQueryCallData extends MongoCallDataRawWrapper {
 ...
 class  ReadBlocksByQuery extends MongoEndPoint {

     function call() {
        /* @var ReadBlocksByQueryCallData */
        $parameters=$this->getModel();
        $db=$this->getDB();
        /* @var \MongoCollection */
        $collection = $db->blocks;
      $q = $parameters->getValueForKey(ReadBlocksByQueryCallData::query);
       if(!isset($q)){
           return new JsonResponse(VOID_RESPONSE,417);
       }
       $f=$parameters->getValueForKey(ReadBlocksByQueryCallData::result_fields);
        try {
            $r=array();
            if(isset($f)){
            ...
            // Sort ?
            $s=$parameters->getCastedDictionaryForKey(ReadBlocksByQueryCallData::sort);
