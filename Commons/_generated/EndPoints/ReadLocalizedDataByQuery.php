...
class  ReadLocalizedDataByQueryCallData extends MongoCallDataRawWrapper {
 ...
 class  ReadLocalizedDataByQuery extends MongoEndPoint {

     function call() {
        /* @var ReadLocalizedDataByQueryCallData */
        $parameters=$this->getModel();
        $db=$this->getDB();
        /* @var \MongoCollection */
        $collection = $db->localizedData;
      $q = $parameters->getValueForKey(ReadLocalizedDataByQueryCallData::query);
       if(!isset($q)){
           return new JsonResponse(VOID_RESPONSE,417);
       }
       $f=$parameters->getValueForKey(ReadLocalizedDataByQueryCallData::result_fields);
        try {
            $r=array();
            if(isset($f)){
            ...
            // Sort ?
            $s=$parameters->getCastedDictionaryForKey(ReadLocalizedDataByQueryCallData::sort);
